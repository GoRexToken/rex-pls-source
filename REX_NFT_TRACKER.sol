...
// File: REX_NFT_TRAIT.sol
// Essential functions: Copyright 2023 "A Bankteller Production"
// modified for PREX
...
struct REXNFTRarityScore {
    uint tokenId;
    uint score;
}


contract PlsRexNftTraitTracker {
    using Counters for Counters.Counter;
    enum Trait { Rare, Hue, Sat, Lum }

    mapping (uint => uint8) private tokenTracker;

    mapping (uint => REXNFTTraits) private tokenTraits;
    ...
    function add(uint batchSize) external {
        require(batchSize > 0, "batchSize must be non-zero");
    ...
    function getInfo(uint tokenId)
        public view returns (uint rare, uint hue, uint sat, uint lum, uint score)
    {
        uint totalSupply = nft.totalSupply();
    ...
    function getScores(uint start, uint end)
        external view returns (REXNFTRarityScore[] memory scores)
    {
        uint totalSupply = nft.totalSupply();
        end = (end == 0) ?  totalSupply : end;  // scan from start to total supply when the end is not specified
        require(end > start && start > 0 && end <= totalSupply, "range out of bounds");
        uint length = end - start + 1;
        scores = new REXNFTRarityScore[](length);
        uint score;
        for (uint i = 0; i < length; i++ ) {
            (, , , , score) = getInfo(start);
            scores[i] = REXNFTRarityScore(start, score);
            start++;
        }
    }
    ...
    function attributes(uint tokenId, address collection)
        private pure returns (uint rare, uint hue, uint sat, uint lum)
    {
        // we don't want to traverse the hue spectrum linearly
