...
// File: REX_NFT_MARKET.sol
// Contract mechanics: Copyright 2023 "A Bankteller Production"
// modified for REX
...
contract plsRexNftMarket is Ownable, ERC721Holder, ReentrancyGuard  {
    ...
    /// @dev Create a deposit dependent staking contract for an nft collection and the REX token
    constructor(IREX _REX, IERC721 _NFT) {
        rex = _REX;
        nft = _NFT;
    }
    ...
    /// @notice event emitted when a user has listed a nft
    event Listed(address owner, uint256 tokenId);

    /// @notice event emitted when a user has delisted a nft
    event Delisted(address owner, uint256 tokenId);
    ...
    function marketPrice(uint256 tokenId) public view returns (uint256) {
        return (INFT(address(nft)).plsStored(tokenId) * 90) / 100;
    }

    /// @dev Retrieve the price
    function mintPrice() public view returns (uint256) {
        return INFT(address(nft)).price();
    }
    ...
    function tokensSoldOfOwner(
     ...
     /// @dev The percentage of the user's balance in relation to the totalSupply, with 1E18 precision
    ...
    /// @dev Stakes one token if owned by the sender
    function stake(uint256 tokenId) public {
        _stake(msg.sender, tokenId);
        txs++;
    }

    /// @dev Unstakes one token if owned by the sender
    ...
    function buy(address _user, uint256 _tokenId) nonReentrant external payable {
        ...
        uint256 _price = marketPrice(_tokenId);

        require(_owner != _user, "user must not be the owner");
        require(_owner != address(0), "token must be staked");
        require(_user != address(0), "not possible");
        require(msg.value == _price, "insufficient funds for purchase");

        (, uint256 _royalty) = INFT(address(nft)).royaltyInfo(_tokenId, _price);
        ...
        // instabuy for treasury
        rex.buyForTreasury{value: _royalty}();
    ...
    function _stake(address _user, uint256 _tokenId) internal {
        ...
        require(staker.tokenIds.length == 0, "Only one");
